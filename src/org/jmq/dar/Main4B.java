	...
	static public void createTrainFile(String roomA, History historyA,int posS,  History historyB, int day) throws Exception{
		...
		for(int indexSensorA:historyA.getCols(Arrays.asList("#S#"))) {
			...
			for(long tA:tAs) {
				...
				for(int i=0;i<10;i++)
					writerBefore.write("  "+historyA.data.get(historyA.getLongT2(tA,-i)).getDegree(indexSensorA));
				writerBefore.write("\n");
				count++;
			}
			...
			for(long tA:tAs) {
				...
				for(int i=0;i<10;i++)
					writerAfter.write("  "+historyA.data.get(historyA.getLongT2(tA,i)).getDegree(indexSensorA));
				writerAfter.write("\n");
				count++;
			}
			System.out.println("#"+count);
